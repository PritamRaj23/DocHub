...
const Appointments = () => {
    ...
    const fetchAppointments = async () => {
        try {
            const response = await Api.get(`doctor/appointments/${profile.email}`);
    ...
    const updateAppointmentStatus = async (updatedAppointment) => {
        try {
            const response = await Api.put(`/doctor/editAppointmentStatus`, updatedAppointment);
    ...
    const handleUnmark = async () => {
        const updatedAppointment = { ...selectedAppointment, status: "rejected" };
        const result = await updateAppointmentStatus(updatedAppointment);
        if (result) {
            setAppointments((prev) =>
                prev.map((appointment) =>
                ...
                {showModal && selectedAppointment && (
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <p>
                                <strong>Client Name:</strong> {selectedAppointment.client.name}
                            </p>
                            <p>
                                <strong>Client Email:</strong> {selectedAppointment.client.email}
                            </p>
                            <p>
                                <strong>Client Phone:</strong> {selectedAppointment.client.phone}
                            </p>
                            <p>
